...
def remove_symbol_change_float(df):
    ...
    for i in dollar_list:
        df[i] = df[i].str.replace('$', '').str.replace(',','').str.strip().astype(float)
...
def portfolio_comparison(df):
    ...
    bmi_percent = [0.081, 0.1090, 0.0760, 0.0450, 0.1620, 0.1150, 0.1150, 0.1720, 0.0490, 0.0420, 0.034]
    buffett_pct = [0.027, 0.0191, 0.1426, 0.0053, 0.4332, 0.0148, 0.0421, 0.3100, 0.0030, 0.0029, 0.000]
    ...
    self_df['Percent'] = (self_df['Market Value']/self_df['Market Value'].sum()).round(4)
    self_df = self_df.drop('Market Value',axis=1)
    self_df['Base']='Current'    
    
    concat_df = pd.concat([bmi_df,buf_df,self_df], join='outer', ignore_index=True)
    
    df = bmi_df.merge(buf_df)
    df = self_df.merge(bmi_df.merge(buf_df, on='Sector', how='left'),on='Sector',how='right')
    ...
    sns.set(style='white')
    fig = sns.catplot(x="Sector", y='Percent', hue='Base', data=concat_df
                 , kind="bar", height=5, aspect=3, palette=['#D3D3D3','#C0C0C0','C3']
                 , legend_out=True)
...
def show_stock_bond_pct(df):
    ...
    fig, ax = plt.subplots()
    ax.axis('equal')
    mypie, _ = ax.pie(BMS_pct, radius =2.5, labels = BMS_name, labeldistance=0.8, colors=[bl(0.2), gr(0.2)])
    plt.setp( mypie, width=0.8, edgecolor='white')
    mypie2, _ = ax.pie(df['Market Value'], radius=2.5-0.8,labels=labels, labeldistance=0.5, colors= [bl(0.5),gr(0.5), gy(0.3)])
...
def check_dividend_growth(ticker, years = 10):
    ...
    fig = df.loc[periods:].plot.line(title=str(ticker) + " - " + str(years) +"-Year Dividend Growth",grid=False)
